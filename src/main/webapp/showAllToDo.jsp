...
<body>
	...
	<div class="main-content">
		...
		<div
			...
			List<ToDoNote> l = s.createQuery("FROM ToDoNote ORDER BY addedDate DESC").getResultList();
			if (l.isEmpty()) {
			%>
				...
				<div class="col-12 col-md-6 col-lg-4 p-3 text-center mb-3">
					...
					%>
					<hr>
					<a class="btn btn-danger" href="DeleteToDoServlet?id=<%=td.getId()%>" role="button">Delete</a>
					<a class="btn btn-primary" href="update.jsp?id=<%=td.getId() %>" role="button">Edit</a>
				</div>
				<%
				}%>
			<div class="col-12 justify-content-center mx-auto p-4 add-shadow text-center"><a class="mx-auto btn btn-outline-info" href="addToDo.jsp">Add new note</a></div>
			<% } s.close();%>
		</div>
	</div>
